...
import { FiPackage, FiCalendar, FiClock, FiCheckCircle, FiUpload, FiImage, FiCamera, FiEye } from 'react-icons/fi';
...
function MyOrders() {
  ...
  const handleUploadScreenshot = async (orderId, file) => {
    ...
    try {
      toast.loading("Uploading proof...", { id: "uploading" });
      ...
      toast.error(error.response?.data?.message || "Upload failed", { id: "uploading" });
    }
  };
                  ...
                  <div className="p-4 sm:p-5 border-b border-slate-50 bg-slate-50/10">
                    ...
                    <div className="flex items-center gap-3 sm:gap-4 text-slate-500">
                      ...
                      <div className="flex items-center gap-1.5 ml-auto">
                        <span className={`px-2 py-0.5 rounded-md size10 degular-bold uppercase tracking-tight ${order.paymentMethod === "UPI"
                        ...
                        {order.paymentScreenshot ? (
                          <div className="relative group rounded-2xl overflow-hidden border border-slate-200 aspect-video bg-slate-50 flex items-center justify-center">
                            ...
                            <div className="absolute top-3 left-3 bg-white/90 backdrop-blur-sm px-2.5 py-1 rounded-lg flex items-center gap-1.5 shadow-sm">
                              ...
                              <span className="size11 degular-bold text-slate-700">Proof Uploaded</span>
                            </div>
                          </div>
                        ) : (
                          <div className="bg-indigo-50/50 border border-dashed border-indigo-200 rounded-2xl p-4 flex flex-col items-center justify-center gap-3">
                            ...
                            <div className="text-center">
                              <p className="size14 degular-bold text-indigo-900 leading-none">Upload UPI Proof</p>
                              <p className="size12 text-indigo-600/70 mt-1 font-medium italic"> Screenshot of your payment</p>
                            </div>
                            <label className="w-full">
                              <input
                                ...
                                accept="image/*"
                                className="hidden"
                                onChange={(e) => handleUploadScreenshot(order._id, e.target.files[0])}
                              />
                              <div className="w-full py-2.5 bg-indigo-600 text-white rounded-xl size13 degular-bold flex items-center justify-center gap-2 shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all cursor-pointer">
                  ...
                  <div className="p-4 sm:p-5 pt-0 mt-auto">
                    <div className="pt-4 sm:pt-5 border-t border-slate-100 flex justify-between items-center">
                      <div className="flex flex-col">
                        <span className="size12 text-slate-500 degular-regular whitespace-nowrap">Total Amount</span>
                        <span className={`px-2 py-0.5 rounded-md size10 degular-bold uppercase tracking-tight w-fit mt-1 ${order.paymentStatus === "Paid"
