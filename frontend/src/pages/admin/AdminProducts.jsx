    ...
    <motion.div
      ...
      <div className="flex flex-col grow px-1">
        <div className="flex justify-between items-start mb-2 gap-2">
          <h3 className="size16 degular-semibold text-slate-800 truncate">
            {product.name}
          </h3>
          <span className="size16 degular-semibold text-indigo-600 shrink-0">
            ₹{product.price}
          </span>
        </div>
        ...
        <div className="mt-auto flex items-center justify-between pt-3 border-t border-slate-50">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="size20 degular-semibold text-slate-600">
              {product.stock}
            </span>
          </div>
        </div>
...
const AdminProducts = () => {
  ...
  const handleEditProduct = (product) => {
    ...
    setImagePreview(
      product.image ? `${import.meta.env.VITE_BASE_URL}${product.image}` : null,
    );
  ...
  return (
    <section className="min-h-screen pb-20 pt-8 px-5 sm:px-6">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <header className="md:mb-10 mb-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between md:gap-6 gap-3 mb-6 md:mb-8">
            <div>
        ...
        {isLoading ? (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
            {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
              <div
                key={i}
                className="aspect-[3/4] rounded-[32px] bg-slate-100 animate-pulse"
              />
            ))}
          </div>
