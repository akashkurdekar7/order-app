    ...
    return (
        ...
        >
            <div className="flex justify-between items-start">
                <div className={`p-3 rounded-2xl ${colors.bg} flex items-center justify-center`}>
                    <Icon size={20} className={colors.text} />
                </div>
                <div className="flex items-center gap-1 text-emerald-500 bg-emerald-50 px-2 py-1 rounded-full">
                    <FiArrowUpRight size={12} />
                    <span className="size11 degular-bold">Live</span>
                </div>
            </div>
            <div>
                <span className="size12 degular-bold text-slate-400 uppercase tracking-widest">{title}</span>
                <h3 className="size28 degular-bold text-slate-800 mt-1">{value}</h3>
...
const AdminSales = () => {
    ...
    const handleExportCSV = () => {
        ...
        setTimeout(() => {
            try {
                const headers = ["Phone Number", "Shop Name", "Vendor Name", "Month", "Total Orders", "Total Sales (INR)", "Balance Due (INR)"];
                const csvContent = [
                    headers.join(","),
    ...
    const totalBalanceDue = sales.reduce((acc, curr) => acc + curr.totalBalance, 0);
    const totalOrdersCount = sales.reduce((acc, curr) => acc + curr.totalOrders, 0);
    ...
    const busyMonth = Object.entries(monthStats).sort((a, b) => b[1] - a[1])[0]?.[0] || "N/A";

    const filteredSales = sales.filter(item => {
        const matchesSearch = item.shopName.toLowerCase().includes(searchTerm.toLowerCase()) ||
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
                        <div>
                            <div className="flex items-center gap-2 text-slate-400 mb-1">
                                ...
                                <span className="size12 degular-bold uppercase tracking-widest">Revenue Intelligence</span>
                            </div>
                            <h1 className="size32 degular-bold text-slate-800">Sales Dashboard</h1>
                        </div>
                        ...
                        <table className="w-full text-left">
                            <thead>
                                <tr className="bg-slate-50/50 border-b border-slate-100">
                                    <th className="px-8 py-5 size11 degular-bold text-slate-400 uppercase tracking-widest">Shop Information</th>
                                    <th className="px-8 py-5 size11 degular-bold text-slate-400 uppercase tracking-widest">Reporting Period</th>
                                    <th className="px-8 py-5 size11 degular-bold text-slate-400 uppercase tracking-widest text-center">Orders</th>
                                    <th className="px-8 py-5 size11 degular-bold text-slate-400 uppercase tracking-widest text-right">Balance</th>
                                    <th className="px-8 py-5 size11 degular-bold text-slate-400 uppercase tracking-widest text-right">Revenue</th>
                                </tr>
                            </thead>
                                ...
                                {isLoading ? (
                                            ...
                                            >
                                                <td className="px-8 py-6">
                                                    <div className="flex flex-col">
                                                        <span className="size15 degular-bold text-slate-800 group-hover:text-indigo-600 transition-colors">{item.shopName}</span>
                                                        <div className="flex items-center gap-2 mt-1">
                                                            <span className="size13 text-slate-500 font-medium">{item.personName}</span>
                                                            <span className="text-slate-300">•</span>
                                                            <span className="size13 text-slate-400">{item.phone}</span>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="px-8 py-6">
                                                    <div className="flex items-center gap-2 text-slate-600">
                                                        ...
                                                        <span className="size14 degular-semibold">{item.month}</span>
                                                    </div>
                                                </td>
                                                <td className="px-8 py-6 text-center">
                                                    <span className="px-3 py-1 rounded-full bg-slate-100 text-slate-600 size13 degular-bold">
                                                        {item.totalOrders}
                                                    </span>
                                                </td>
                                                <td className="px-8 py-6 text-right">
                                                    <span className={`size15 degular-bold ${item.totalBalance > 0 ? "text-red-500" : "text-emerald-500"}`}>
                                                        ₹{item.totalBalance.toLocaleString()}
                                                    </span>
                                                </td>
                                                <td className="px-8 py-6 text-right">
                                                    <div className="flex flex-col items-end">
                                                        <span className="size16 degular-bold text-slate-800">₹{item.totalSales.toLocaleString()}</span>
                                                        <span className="size11 degular-bold text-emerald-500 uppercase tracking-tighter mt-1">Settled</span>
                                                    </div>
                                                </td>
                                            </motion.tr>
                                        ))}
                                    ...
                                    <tr>
                                        <td colSpan="4" className="py-24 text-center">
                                            ...
                                            <p className="size16 degular-semibold text-slate-400">No sales records found for this selection.</p>
                                        </td>
                                    </tr>
                                )}
                            </tbody>
