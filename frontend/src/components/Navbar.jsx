...
import { FiLogOut, FiMenu, FiX, FiShoppingBag, FiUser, FiPackage, FiHome } from "react-icons/fi";
...
const Navbar = () => {
  ...
  return (
    <>
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 px-4 sm:px-6 lg:px-8 py-4 ${
        scrolled ? "mt-0" : "mt-2"
      }`}>
        <div className={`max-w-7xl mx-auto h-16 sm:h-20 glass-effect rounded-[24px] sm:rounded-[32px] flex items-center justify-between px-6 sm:px-10 transition-all duration-500 border border-white/40 shadow-xl ${
          scrolled ? "shadow-slate-200/50" : "shadow-transparent"
        }`}>
          {/* Logo */}
          <Link to={role === 'admin' ? '/admin' : '/home'} className="flex items-center gap-3 group">
            <div className="w-10 h-10 sm:w-12 sm:h-12 bg-slate-900 rounded-2xl flex items-center justify-center shadow-lg shadow-slate-200 transition-transform group-hover:scale-110">
              <span className="text-white degular-bold size20 sm:size24">W</span>
            </div>
            <div className="flex flex-col">
              <span className="degular-bold size16 sm:size18 text-slate-800 leading-none">Wholesale</span>
              <span className="degular-bold size12 sm:size14 text-indigo-600 leading-none">Market</span>
            </div>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-2">
            ...
            <motion.button
              whileTap={{ scale: 0.95 }}
              onClick={logout}
              className="flex items-center gap-2 px-5 py-2.5 bg-red-50 text-red-600 rounded-xl size14 degular-bold hover:bg-red-100 transition-colors shadow-sm"
            >
          ...
          <button
            ...
            className="md:hidden w-10 h-10 rounded-2xl bg-slate-50 flex items-center justify-center text-slate-600 hover:text-indigo-600 transition-all border border-slate-200"
          >
            <FiMenu size={24} />
          </button>
        </div>
          ...
          <div className="fixed inset-0 z-60 overflow-hidden">
            ...
            >
              ...
              <div className="p-8 pb-10 flex justify-between items-center border-b border-slate-50 bg-slate-50/30">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 bg-slate-900 rounded-[20px] flex items-center justify-center">
                    <span className="text-white degular-bold size24">W</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="degular-bold size18 text-slate-800 leading-none">Menu</span>
                    <span className="degular-bold size14 text-indigo-600 uppercase tracking-widest mt-1">Wholesale</span>
                  </div>
                </div>
                <button 
                  onClick={() => setIsMenuOpen(false)}
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          ...
                          <FiX className={`rotate-45 ${isActive ? "text-white/40" : "text-slate-300"}`} size={20} />
                        </Link>
                      </motion.div>
                    );
                  })}
              ...
              <div className="p-6 sm:p-8 bg-slate-50/30 border-t border-slate-50">
                ...
                >
                  <FiLogOut size={22} /> Close Account Session
                </motion.button>
              </div>
            </motion.div>
          </div>
